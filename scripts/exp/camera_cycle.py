...
Prints CYCLE / REPS lines; events go to $FM_LOGS/bench_camera/events.jsonl (per-instance skill_instance records).
    ...
    python scripts/exp/camera_cycle.py <config_index> [--act] [--reps R] [--task 0] [--workers 4] [--cycles 1]
"""
...
def main():
    ...
    ap.add_argument("config_index", type=int)
    ...
    tag = f"plus_camera_{a.config_index}" + ("_act" if a.act else "")
    ...
    for _ in range(a.cycles):
        ...
        print(f"CYCLE {tag} cons={c.consolidation_id} rollouts={c.rollouts} wall={c.wallclock_s:.0f}s "
    ...
    print(f"INCUMBENT {tag} v{inc.version} {json.dumps(inc.params)}", flush=True)
    if a.skip_eval:
        close_pool(); return
    seeds = [5040 + i + 50 * (r + a.rep_offset) for r in range(a.reps) for i in range(10)]
    arms = {"BM-1": dataclasses.replace(tmpl, arm="A"),
    ...
    for name, c in arms.items():
        ...
        print(f"REPS {tag} {name}: {k}/{n} = {100 * k / n:.0f}% [{100 * lo:.0f},{100 * hi:.0f}] steps {res[name]['mean_steps']:.0f}", flush=True)
