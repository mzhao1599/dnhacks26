...
def beat(name, cfg, cache):
    ...
    dst = f"{VID}/cam_{name.split(' ')[0]}_{'ok' if ep.outcome.env_success else 'fail'}.mp4"
...
def main():
    ...
    cache = {}
    beat("BM-0 stock camera · raw VLA", base, cache)
    beat("BM-1 camera bumped · raw VLA", pert, cache)
    if inc and inc["version"] >= 2:
        beat(f"BM-3 camera bumped · consolidated v{inc['version']}", dataclasses.replace(pert, arm="B", s3_params=inc["params"]), cache)
    else:
