...
def test_time_scale_halves_chunk_in_shim():
    cs = S3Params(time_scale=2.0).to_constraint_set()
...
def test_time_scale_shortens_scripted_episode():
    ...
    def lift_step(ts):
        env, tracker, shim = make(ScriptedPolicy(bias_z=0.0), S3Params(time_scale=ts).to_constraint_set())
...
def test_blend_grasp_offset_shifts_closing_point():
    ...
    base, env0, shim0 = closing(S3Params.identity().to_constraint_set())
    ...
    rel, env, shim = closing(S3Params(grasp_offset_z=-0.015).to_constraint_set())
...
def test_blend_never_overrides_and_only_in_approach():
    cs = S3Params(pregrasp_height=0.08).to_constraint_set()             # disagrees with the stub's 0.05 hover
...
def test_blend_direction_pulls_toward_waypoint():
    cs = S3Params(approach_offset_xyz=np.array([0.03, 0.0, 0.0]), pregrasp_height=0.05).to_constraint_set()
...
def test_cone_in_blend_mode_after_waypoint_only():
    cs = S3Params(approach_cone_deg=20.0).to_constraint_set()
...
def test_envelope_clamps_even_when_disabled():
    ...
    env, tracker, shim = make(ScriptedPolicy(bias_z=0.015), S3Params(grasp_offset_z=-0.015).to_constraint_set())
